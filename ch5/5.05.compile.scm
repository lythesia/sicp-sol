...
(define (compile-assignment exp target linkage)
  (let ((var (assignment-variable exp)) (get-value-code (compile (assignment-value exp) 'val 'next)))
    (end-with-linkage
      linkage
      (preserving
...
(define (compile-definition exp target linkage)
  (let ((var (definition-variable exp)) (get-value-code (compile (definition-value exp) 'val 'next)))
    (end-with-linkage
      linkage
      (preserving
        '(env)
...
(define (compile-if exp target linkage)
  (let ((true-br (make-label 'true-branch)) (false-br (make-label 'false-branch)) (after-if (make-label 'after-if)))
    (let ((consequent-linkage (if (eq? linkage 'next) after-if linkage))) ; if after consequent is next, then we go after `after-if`
      (let ((p-code (compile (if-predicate exp) 'val 'next))  ; match (1)
            (c-code (compile (if-consequent exp) target consequent-linkage))
            (a-code (compile (if-alternative exp) target linkage))) ; whether linkage is 'next or not, still linkage so we dont need `let binding` like consequent-linkage 
        (preserving
          '(env continue)
          p-code
